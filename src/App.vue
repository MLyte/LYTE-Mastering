...
onMounted(async () => { try { cleanup.push(await listen<number>("mastering-progress", e => { if (!busy.value) return; if (mode.value === "auto") { if (e.payload + 5 < engineProgress.value) autoRender.value += 1; engineProgress.value = e.payload; progress.value = Math.min(99, Math.round(((autoRender.value + e.payload / 100) / 9) * 100)); } else progress.value = Math.max(progress.value, Math.min(90, Math.round(e.payload * .9))); })); cleanup.push(await getCurrentWebview().onDragDropEvent(e => { dragging.value = !busy.value && (e.payload.type === "over" || e.payload.type === "enter"); if (e.payload.type === "drop" && !busy.value && !loading.value) { dragging.value = false; if (e.payload.paths.length !== 1) fail("Please drop one audio file at a time."); else void load(e.payload.paths[0]); } })); connected.value = true; } catch { error.value = "Desktop connection unavailable. Launch LYTE using npm run dev."; } const main = document.querySelector("main"); resizeObserver = new ResizeObserver(scheduleWindowFit); if (main) resizeObserver.observe(main); scheduleWindowFit(); });
