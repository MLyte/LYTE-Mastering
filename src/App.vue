...
function resetResults() { auto.value = null; manual.value = null; selected.value = ""; exportedVariantIds.value = []; exportedAacVariantIds.value = []; exporting.value = false; previewPlayer.value?.pause(); }
function resetProgress() { progress.value = 0; autoRender.value = 0; engineProgress.value = 0; }
function restart() { if (busy.value || loading.value) return; track.value = null; reference.value = null; sourceWaveform.value = null; referenceWaveform.value = null; status.value = "Idle"; error.value = ""; output.value = ""; resetProgress(); resetResults(); }
async function load(path: string) { if (busy.value || loading.value) return; loading.value = true; error.value = ""; output.value = ""; resetProgress(); resetResults(); try { track.value = await invoke<Track>("inspect_track", { path }); sourceWaveform.value = await invoke<Waveform>("inspect_waveform", { path }); autoSettings.sourceStart = 0; status.value = "Ready"; } catch (e) { track.value = null; sourceWaveform.value = null; fail(e); } finally { loading.value = false; } }
...
async function master() {
  ...
  primeCompletionSound(); status.value = "Processing"; resetProgress(); error.value = ""; output.value = ""; resetResults();
  try {
    if (mode.value === "auto") {
...
function preview(path: string, lufs: number, start = 0) {
  ...
  if (!player) return fail("Preview is not ready yet. Please try again.");
  const values = auto.value ? [auto.value.source.integratedLufs, ...auto.value.variants.map(v => v.measurements.integratedLufs), auto.value.referenceSegment?.integratedLufs].filter((value): value is number => Number.isFinite(value)) : [lufs];
  player.pause();
  player.onloadedmetadata = () => { player.currentTime = Math.max(0, start); };
  player.onerror = () => fail("Preview could not load this local audio file. Run mastering again or choose the track again.");
  player.src = convertFileSrc(path);
  player.volume = Math.pow(10, Math.min(0, Math.min(...values) - lufs) / 20);
  player.load();
  // Keep play() in the click handler: WebView2 may reject a call deferred to
  // loadedmetadata as autoplay, even though the user pressed Play.
  void player.play().catch(() => fail("Preview could not start. Check that another app is not using the audio device, then try again."));
}
...
onMounted(async () => { try { cleanup.push(await listen<number>("mastering-progress", e => { if (!busy.value) return; if (mode.value === "auto") { if (e.payload + 5 < engineProgress.value) autoRender.value += 1; engineProgress.value = e.payload; progress.value = Math.min(99, Math.round(((autoRender.value + e.payload / 100) / 9) * 100)); } else progress.value = Math.max(progress.value, Math.min(90, Math.round(e.payload * .9))); })); cleanup.push(await getCurrentWebview().onDragDropEvent(e => { dragging.value = !comparisonOpen.value && !busy.value && (e.payload.type === "over" || e.payload.type === "enter"); if (e.payload.type === "drop" && !comparisonOpen.value && !busy.value && !loading.value) { dragging.value = false; if (e.payload.paths.length !== 1) fail("Please drop one audio file at a time."); else void load(e.payload.paths[0]); } })); connected.value = true; } catch { error.value = "Desktop connection unavailable. Launch LYTE using npm run dev."; } const main = document.querySelector("main"); resizeObserver = new ResizeObserver(scheduleWindowFit); if (main) resizeObserver.observe(main); scheduleWindowFit(); });
watch(stage, async () => { await nextTick(); scheduleWindowFit(); }, { flush: "post" });
watch(comparisonOpen, async () => { await nextTick(); scheduleWindowFit(); }, { flush: "post" });
