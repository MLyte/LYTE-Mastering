...
Le mode **Auto — Hard Techno** part d’une cible prudente calculée depuis le niveau du morceau, puis monte par pas de 1 dB. Chaque rendu repart de la même copie PCM 24 bits du source. La recherche s’arrête quand le gain de niveau devient inférieur à 0,3 LU, qu’un garde-fou est franchi ou que la limite de −4 dB est atteinte ; au voisinage d’un garde-fou, elle teste un pas intermédiaire de 0,5 dB. Elle rejoue ensuite la meilleure cible avec `Preserve bass` activé. L’intensité reste fixée à 1,00. Ces cibles ne sont pas des promesses de LUFS : après chaque rendu, FFmpeg mesure les LUFS intégrés, le true peak, le facteur de crête et une estimation de la proportion d’énergie entre 30 et 150 Hz. Le fichier exportable est aussi encodé localement en AAC 256 kbit/s puis remesuré : ce contrôle détecte les crêtes créées par cet encodage, sans prétendre reproduire exactement le traitement de chaque plateforme.

L'export retenu reprend aussi le nom de base du morceau : `<original>_hard-techno_-4.5dB_bass-on_auto.wav`.

En mode Auto, LYTE applique un contrôle dynamique léger sous 150 Hz et un soft clip de 0,5 dB. Le mode Manual conserve les réglages correspondants. Il atténue ensuite les rendus qui dépassent −1 dBTP, puis mesure à nouveau le fichier exportable. Une variante Auto n’est admissible que si la perte de facteur de crête reste au plus à 3 dB, si la variation du grave reste dans ±2 dB et si le test AAC reste à 0 dBTP ou moins. L’indicateur « Technical balance » combine niveau, true peak, dynamique, grave et sécurité codec pour classer les variantes admissibles ; il s’agit d’une heuristique de comparaison, pas d’une mesure de qualité professionnelle. Si aucune variante ne respecte tous les seuils, LYTE présente seulement le candidat le moins agressif et conseille de conserver la source ou de revoir le mix.

Les variantes et la copie de préécoute restent dans un dossier temporaire local à la session. La préécoute atténue les pistes les plus fortes pour faciliter une comparaison à volume égal ; elle ne modifie jamais le master. **Export selected master** copie uniquement le choix vers le dossier du morceau et ajoute un suffixe numérique en cas de conflit. Le nom conserve la cible, l'intensité et le réglage des basses : modifier un réglage crée donc un master distinct. Les masters existants avec les mêmes réglages ne sont jamais écrasés ; les fichiers partiels sont temporaires. `Open folder` utilise uniquement le dernier résultat enregistré côté Rust. Les réglages sont validés côté Rust et mémorisés localement côté Vue. Un seul traitement à la fois ; la fermeture normale de la fenêtre est empêchée pendant le traitement (pas d'annulation dans ce MVP).
