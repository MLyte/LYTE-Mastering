...
Le mode **Auto — Hard Techno** vise un niveau de sortie mesuré entre **−9 et −2 LUFS** (−5 LUFS par défaut). Le bouton « Use integrated Hard Techno level » sélectionne −2,9 LUFS. Il rend trois masters à la même cible : **Fidèle** (intensité 0,65, grave préservé, sans compression additionnelle du grave ni soft clip de profil), **Dense** (0,85, grave préservé, soft clip 0,5 dB) et **Agressif** (1,00, grave dynamique, soft clip 1 dB). Chaque profil utilise un passage moteur, puis une calibration finale bornée pour se rapprocher de ±0,3 LUFS ; si cela échoue, LYTE conserve son résultat le plus proche et explique l’écart, sans relancer le moteur avec une consigne plus forte.

Le grave dynamique utilise un crossover complémentaire à 150 Hz pour éviter une annulation à la recombinaison. Les intermédiaires de finition restent en virgule flottante. Après cette finition, une calibration du gain avec écrêtage doux indépendant du grave et du reste du spectre, suréchantillonné ×4 lorsque les crêtes le nécessitent, mesure chaque sortie PCM 24 bits, corrige son true peak puis ajuste le gain vers la cible (huit essais au maximum, toujours depuis le même intermédiaire). La saturation de cette finition est bornée à 12 dB au-dessus de la marge de crête disponible, indépendamment du niveau initial ; la recombinaison dispose de 3 dB de marge et d’un écrêtage instantané borné à 3 dB, sans enveloppe de gain large bande, afin de ne pas faire remonter le lead à la coupure des kicks. Une cible impossible à atteindre dans cette limite reste signalée dans les diagnostics, au lieu de forcer davantage la saturation. Le true peak final reste plafonné à −1 dBTP. Les différences de facteur de crête et de grave deviennent des compromis affichés par profil ; elles ne font plus disparaître un master dense. Chaque rendu est aussi encodé localement en AAC 256 kbit/s puis remesuré. Un dépassement AAC est signalé comme risque de diffusion, sans masquer le fichier.

Une référence WAV, FLAC ou MP3 choisie par l’utilisateur reste locale. Sans sélection, une référence Hard Techno incluse est utilisée sur un passage fixe de 30 secondes. LYTE compare le facteur de crête, le grave 30–150 Hz et quatre bandes d’énergie ; elle ne laisse jamais le niveau global de la référence modifier la cible LUFS demandée. Cette comparaison classe les trois profils, sans copier la courbe tonale de la référence.

Les sources, références converties et variantes restent dans un dossier temporaire local de session. La préécoute aligne les niveaux pour une comparaison plus juste et chaque profil peut être exporté séparément : `<original>_hard-techno_dense_target--5.0LUFS_auto.wav`, avec suffixe numérique en cas de conflit. Les masters existants ne sont jamais écrasés.
